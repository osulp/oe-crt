...
export class DataTileComponent implements OnInit, OnDestroy, OnChanges {
    ...
    @Input() selectedPlaceTypeInput: any;
    @Input() related: boolean;
    @Input() collections: any[];
    @Input() _selectedYear: any;
    @Input() drillDowns: any[];
    @Input() drillDownCategories: any; //Total /race/ethnicity, etc
    ...
    private ddOnlyForCounty: boolean;
    private ddOnlyForState: boolean;
    ...
    saveInstance(chartInstance: any) {
        ...
        this._indicatorDescService.getIndicator(this.indicator.replace(/\+/g, '%2B').replace(/\&/g, '%26').replace(/\=/g, '%3D')).subscribe(
            (indicatorDesc: any) => {
                console.log('ralph,', this.tileType, indicatorDesc);
    ...
    checkLoadGeoJSON() {
        ...
        if (selPTCheck.length === 0) {
            ...
            console.log(geoJSON_to_load, this.geoJSONStore);
        ...
        let bndryChk = geoJSON_to_load.filter(layer => { return this.translatePlaceTypes(layer) === 'Places' || this.translatePlaceTypes(layer) === ''; });
        ...
        //check if geoAoi specified and load place type
        if (this.geoAoi) {
            if (this.geoAoi.geoid && this.tileType === 'map') {
                geoJSON_to_load.push(this.translatePlaceTypes(this.geoAoi.geo_type));
            }
        }
        ...
        if (geoJSON_to_load.length > 0) {
            ...
            console.log('filtered gtl', geoJSON_to_load_filtered, geoJSON_to_load);
    ...
    getGeoJSON(placeTypeToLoad: any[]) {
        ...
        if (this.placeTypeGeoYears !== 'undefined') {
            for (var pt of placeTypeToLoad) {
                console.log('wtf', this.placeTypeGeoYears, this.translatePlaceTypes(pt));
                this._geoService.getByPlaceType(this.translatePlaceTypes(pt), this.placeTypeGeoYears).subscribe(
                    (data: any) => {
                        ...
                        if (data.length > 0) {
                            ...
                            console.log('got geojson, updated data store and checking place type to get indicator data', this.selectedPlaceType, data);
    ...
    processCombinedData(data: any, drilldown?: any) {
        ...
        if (!data.Metadata[0].isPreCalc && data.Metadata[0].Variable_Represent.trim() !== 'Text') {
            ...
            for (var group of groups) {
                ...
                //find the variables to process
                var variablesToCombine: any[] = [];
                if (drilldown) {
                    variablesToCombine = this.drillDowns
                        .map((dd: any) => dd.variable)
                        .filter((val: any, idx: number, self: any) => {
                            return self.indexOf(val) === idx;
                        });
                } else {
                    variablesToCombine.push(group[0].Variable);
                }
                console.log('worldcup', variablesToCombine);
                variablesToCombine.forEach((vtc: any) => {
                    var notCombined = false;
                    let combinedGroupData: any = new Object;
                    combinedGroupData.community = group[0].GroupName;
                    combinedGroupData.Variable = vtc ? vtc : group[0].Variable;
                    ...
                    for (var year of data.Years) {
                        ...
                        for (var place of group) {
                            //console.log('goal', place, combinedData);
                            let combinedPlaceData: any[] = [];
                            if (drilldown) {
                                combinedPlaceData = combinedData.data;
                            } else {
                                combinedPlaceData = combinedData.Data;
                            }
                            let placeData = combinedPlaceData.filter((pData: any) => {
                                // console.log('let it go', pData, place, vtc);
                                return (pData.geoid === place.ResID) && (vtc ? pData.Variable === vtc : true);
                            });
                            console.log('chickens', placeData, vtc);
                    ...
                    for (var place of group) {
                        ...
                        //if (drilldown) {
                        //    combinedData.drilldown_combined_data = combinedData.drilldown_combined_data.filter((pData: any) => { return pData.geoid !== place.ResID && pData.community !== place.Name; });
                        //} else {
                        //    combinedData.Data = combinedData.Data.filter((pData: any) => { return pData.geoid !== place.ResID && pData.community !== place.Name; });
    ...
    getDrillDownData(evt: any) {
        ...
        let searchGeoidObj = this.places.filter((p: any) => {
            console.log('chump', p, evt.point.series.options.id.split('<br>')[0].replace(evt.point.series.options.geoid, ''));
            return (isCombined ? p.GroupName : p.Name.trim() === evt.point.series.name)
                || (this.isSchool && p.GeoInfo.length > 0 ? p.GeoInfo[0].School_District.indexOf(evt.point.series.options.id.split('<br>')[0].replace(evt.point.series.options.geoid, '')) !== -1 : false)
                || (p.Desc ? p.Desc.trim().includes(evt.point.series.name) : false);

        });
        console.log('search GeoidObj is:  ', searchGeoidObj);
        let searchGeoid = '';
        searchGeoidObj.forEach((sgo: any, idx: number) => {
            searchGeoid += (idx !== 0 ? ',' : '')
                + (this.ddOnlyForCounty
                    ? sgo.GeoInfo[0].county_geoid
                    : this.isSchool
                        ? evt.point.series.options.geoid
                        : sgo.ResID);
        });

        //searchGeoid = this.isSchool ? searchGeoid === '41' ? "'41','9999'" : searchGeoid : searchGeoid;
        console.log('search Geoid is: ', searchGeoid);
        this.drillDownSeries = {
            id: evt.point.series.options.id,
            geoid: evt.point.series.options.geoid,
            name: evt.point.series.name,
            combined: isCombined
        };
        if (searchGeoid) {
            this.drillDownPlace = this.ddOnlyForCounty && !isCombined ? searchGeoidObj[0].GeoInfo[0].County + ' County' : evt.point.series.name.replace('<br>', '&nbsp;');
            let subtopic = encodeURI(this.placeTypeData.Metadata[0].Sub_Topic_Name);
            this._dataService.getDrilldownIndicatorData(subtopic, searchGeoid).subscribe(
                (data: any) => {
                    ...
                    this.addDrillDownDataToGraphChart();
                },
                (err: any) => console.error(err),
                () => console.log('done loading data for map')
            );
    ...
    addDrillDownDataToGraphChart() {
        console.log('sunlight', this.drillDownSeries, this);
        let drillDownCategory = this.drillDownCategories.filter((ddc: any) => ddc.selected)[0].category;
        let data = this.drillDownData.filter((d: any) => {
            return this.drillDowns.filter((dd: any) => {
                if (drillDownCategory === 'All') {
                    return true;
                } else {
                    return dd.variable === d.Variable && (dd.category === drillDownCategory || dd.category === 'Total');
                }
            }).length > 0;
        });
        let subtopic = this.placeTypeData.Metadata[0].Sub_Topic_Name;
        let ddObj = {
            place: this.isSchool ? this.drillDownSeries.id.replace(this.drillDownSeries.geoid, '')
                : this.drillDownSeries.name,
            subtopic: subtopic,
            data: data,
            Data: data,
            ddOnlyForCounty: this.ddOnlyForCounty,
            ddCounty: this.drillDownPlace.replace(' County', ''),
            Metadata: this.dataStore.indicatorData[this.indicator].crt_db.Metadata,
            Years: this.dataStore.indicatorData[this.indicator].crt_db.Years
        };
        this.isDrilldown = true;
        console.log('panama', this.drillDownData);

        if (this.drillDownSeries.combined) {
            let combinedData = this.processCombinedData(ddObj, true);
            console.log('hotdog', combinedData);
            this.updateDataStore(combinedData.Data, 'indicator', ddObj);
        } else {
            this.updateDataStore(data, 'indicator', ddObj);
        }
        this.processDataYear(ddObj);
        this.selectedYear = ddObj.Years[ddObj.Years.length - this.yearEndOffset - 1];
        ...
        this.chart.xAxis[0].update({
            ...
            tickInterval: this._tickArray.length - (this.yearStartOffset + this.yearEndOffset) > 10 ? 2 : null,
            plotLines: [{
                color: 'gray',
        ...
        if (this.indicator_info.drilldown_indicators[0].drilldown_y_axis_label) {
            this.chart.yAxis[0].update({
                title: {
                    text: this.indicator_info.drilldown_indicators[0].drilldown_y_axis_label
                }
            });
        }
    ...
    onDDCatChange(ddCat: any) {
        console.log('ddCat!', ddCat, this.drillDownCategories);
        this.drillDownCategories.forEach((ddc: any) => {
            ddc.selected = ddc.category === ddCat;
        });
        console.log('ddCat selected', this.drillDownCategories);
        if (this.isDrilldown) {
            this.addDrillDownDataToGraphChart();
        }
    }
        ...
        $(this.elementRef.nativeElement).find('#dateSlider').labeledslider(
            {
                ...
                change: function (event: any, ui: any) {
                    ...
                    if (sliderScope.isCustomChart) {
                        ...
                        let seriesIndex = sliderScope.mapChart.series.length - (sliderScope.geoAoi
                            ? sliderScope.geoAoi.geoid
                                ? 2
                                : 1 : 1);
    ...
    initMapChart() {
        ...
        console.log('checkshit2', series, this.mapChart.series, this.dataStore[this.pluralize(this.selectedPlaceType)].indicatorData[this.indicator].chart_data.place_data, this.getSelectedMapData());
        this.mapChart.series[this.mapChart.series.length - 1].mapData = this.getSelectedMapData();
        this.mapChart.series[this.mapChart.series.length - 1].setData(this.dataStore[this.pluralize(this.selectedPlaceType)].indicatorData[this.indicator].chart_data.place_data);

        if (this.geoAoi) {
            //add geoAoi shape
            console.log('trying to get the geoaoi data to load', this.geoAoi, this.dataStore, this.tileType);
            try {
                let _geoAoiGeoTypeData = this.dataStore[this.translatePlaceTypes(this.geoAoi.geo_type)]
                    ? this.dataStore[this.translatePlaceTypes(this.geoAoi.geo_type)].mapData.features : null;
                if (_geoAoiGeoTypeData !== null) {
                    //clone object to not mutate the original geojson data
                    let geoAoiGeoTypeData = JSON.parse(JSON.stringify(_geoAoiGeoTypeData[_geoAoiGeoTypeData.length - 1]));
                    let geoAOIFeatures: any[] = [];
                    geoAoiGeoTypeData.features.forEach((f: any) => {
                        if (f.properties.GEOID === this.geoAoi.geoid) {
                            console.log('dreamin', f);
                            let geoFeature = f;
                            geoFeature.geometry.type = 'MultiLineString';
                            geoFeature.properties.NAME = f.properties.NAME.replace('_GEOAOI', '') + '_GEOAOI';
                            geoAOIFeatures.push(geoFeature);
                        }
                    });
                    geoAoiGeoTypeData.features = geoAOIFeatures;
                    let mapLineGeoJson = Highcharts.geojson(geoAoiGeoTypeData, 'mapline');
                    var geoAoiSeries = {
                        lineWidth: '3px',
                        color: 'black',
                        name: 'GeoAOI',
                    ...
                    this.mapChart.addSeries(geoAoiSeries);
                    ptSeriesIndexes.push(this.mapChart.series.length - 1);
                }
            } catch (ex) {
                console.log('failled to load geoAoi map data', ex.message);
            }
        }
        ...
        window.setTimeout(function () {
            ...
            if (mapScope.geoAoi) {
                //get point from geoaoi layer.
                mapScope.mapChart.series[mapScope.mapChart.series.length - 1].points[0].zoomTo();
                mapScope.mapChart.mapZoom(2);
            }
        }, 500);
        if (this.indicator_info.Represented_ID === 10) {
    ...
    selectAndOrderMapPaths() {
        console.log('sausage', this.selectedPlaceType, this.mapChart.series, this.geoAoi);
        this.mapChart.hideLoading();
        this.selectedMapPoints = this.mapChart.getSelectedPoints();
        let seriesIndex = this.selectedPlaceType === 'Places' ? 1 : 0;
        if (this.geoAoi) {
            seriesIndex = this.geoAoi.geoid ? 1 : seriesIndex;
        }

        this.mapChart.series[seriesIndex].data.forEach((d: any) => {
            if (this.selectedMapPoints.map((mp: any) => mp.geoid).indexOf(d.geoid) !== -1) {
                d.update({ selected: false });
                d.update({ selected: true });
            }
        });
        let mapPathGroup = $("#highmap path[class*='highcharts-name-']").not("#highmap path[class*='_geoaoi']").parent();
        let mapPaths = $("#highmap path[class*='highcharts-name-']").not("#highmap path[class*='_geoaoi']");
        //console.log('mapPath Stuff', mapPathGroup, mapPaths);
        mapPaths.detach().sort((a: any, b: any) => {
            return parseInt($(a).attr('stroke-width').replace('px', '')) - parseInt($(b).attr('stroke-width').replace('px', ''));
        });
        //console.log('mapPath Stuff2', mapPathGroup, mapPaths);
        mapPathGroup.prepend(mapPaths);
    }
                ...
                if (this.placeTypeData.Metadata.length > 0) {
                    ...
                    this.chart.tooltip.options.formatter = function (): any {
                        ...
                        if (this.series.name.match('Error')) {
                            ...
                            console.log('drilldown geogs', chartScope.drillDownGeogs, chartScope.drillDownGeogs.indexOf('State'), this.point.series.options.geo_type);
                            let ddOnlyForCounty = chartScope.drillDownGeogs.indexOf('County') !== -1;
                            let ddOnlyForState = chartScope.drillDownGeogs.indexOf('State') !== -1;
                            let hasGeoTypePtSeries = this.point.series.options.geo_type ? true : false;

                            chartScope.ddOnlyForState = chartScope.drillDownGeogs.indexOf('State') !== -1
                                ? hasGeoTypePtSeries
                                    ? this.point.series.options.geo_type === 'State'
                                    : false
                                : false;
                            console.log('state only?', chartScope.ddOnlyForState, this, this.point.series.options.geo_type, chartScope.drillDownGeogs);
                            ...
                            return '<span style="fill: ' + this.series.color + ';"> ● </span><span style="font-size:10px"> ' + this.point.series.name + ' (' + this.x + ')</span><br/><span><b>' + displayValue + '</span><br/>' + (chartScope.viewType === 'advanced' ? (drillDownMsg + ddCountyOnlyMsg) : '');
                        }
                    };
                    ...
                    if (this.hasDrillDowns && this.viewType === 'advanced') {
    ...
    addSeriesDataToGraphChart(mapPlaces?: any[], drilldown?: any) {
        ...
        sortedPlaceData.forEach((pd: any, idx: number) => {
            ...
            let pdCommunityName = this.isSchool && !drilldown
                ? pd.Name
                : drilldown
                    ? this.isSchool && drilldown.place === 'Oregon'
                        ? 'Statewide'
                        : drilldown.place.replace(' County', '')
                    : pd.community;

            let pdDDCommunityName = drilldown && !this.isSchool
                ? drilldown.ddOnlyForCounty && !this.isSchool
                    ? drilldown.ddCounty
                    : drilldown.place.replace(' County', '')
                : this.isSchool
                    ? pd.Name
                    : pdCommunityName;
            console.log('pdCOmmunityName : ', pdCommunityName);
            console.log('pdDDCOmmunityName : ', pdDDCommunityName);
            ...
            console.log('scherwma', drilldown, this.dataStore, pdDDCommunityName, pd);
            let data = !drilldown
            ...
            if (addedSeries.indexOf((drilldown ? pdDDCommunityName : pdCommunityName) + pd.geoid) === -1
                ...
                if (addedSeries.indexOf(seriesID) === -1) {
                    ...
                    if (!drilldown && this.hasDrillDowns && this.viewType === 'advanced') {
                        console.log('trdlnik?', drilldown, this.hasDrillDowns, this.viewType, this.chart.series[this.chart.series.length - 1], addedSeries);
        ...
        var extremes = this.chart.yAxis[0].getExtremes();
        console.log('get extremes', extremes, this.chart);
    ...
    processDataYear(drilldown?: any) {
        this.yearStartOffset = this.getStartYear(drilldown);
        this.yearEndOffset = this.getEndYear(drilldown);
        let place_data = [{}];
        let place_data_years: any = {};
        ...
        if (this.isCustomChart) {
            ...
            ptdToProcess.Data.forEach((pData: any) => {
                ...
                let pdyPlace = this.isSchool ? pData.Name : pData.community;

                if (!drilldown) {
                    place_data_years[pdyPlace] = {
                        id: pdyPlace,
                        name: pdyPlace,
                        geoid: pData.geoid,
                        data: year_data,
                        drilldown: this.hasDrillDowns,
                        //geo_type: this.pluralize(this.selectedPlaceType)
                    };
                    place_data_years_moe[pdyPlace] = {
                        id: pdyPlace,
                        name: pdyPlace,
                        geoid: pData.geoid,
                        data: year_data_moe,
                    ...
                    console.log('vat is pdata', pData);
                    place_data_years_dd[pdyPlace + ': ' + pData.Variable] = {
                        id: pdyPlace + ': ' + pData.Variable,
                        name: pdyPlace + ': ' + pData.Variable,
                        geoid: pData.geoid,
                        data: year_data_dd,
                        //geo_type: this.pluralize(this.selectedPlaceType)
                    };
                    place_data_years_moe_dd[pdyPlace + ': ' + pData.Variable] = {
                        id: pdyPlace + ': ' + pData.Variable,
                        name: pdyPlace + ': ' + pData.Variable,
                        geoid: pData.geoid,
    ...
    getStartYear(drilldown?: any) {
        //start at the beggining and move forward until you find data
        console.log('looking for start year data', drilldown);
        let counter = 0;
        let years = drilldown ? drilldown.Years : this.placeTypeData.Years;
        let data = drilldown ? drilldown.Data : this.placeTypeData.Data;
    ...
    getEndYear(drilldown?: any) {
        //start at the end and move back until you find data
        console.log('looking for end year data', drilldown);
        let counter = 0;
        let years = drilldown ? drilldown.Years : this.placeTypeData.Years;
        let data = drilldown ? drilldown.Data : this.placeTypeData.Data;
    ...
    onSelectedMapViewChange(evt: any) {
        console.log('changing map', evt, this.selectedPlaceType);
    ...
    ngOnInit() {
        ...
        this.defaultAdvChartOptions.chart.spacingTop = this.drillDowns ? this.drillDowns.length > 0 ? 200 : this.defaultAdvChartOptions.chart.spacingTop : this.defaultAdvChartOptions.chart.spacingTop;
        //this.defaultChartOptions.title = {
        //    text: this.indicator ? this.indicator : null,
        //    align: this.viewType === 'basic' ? 'left' : null
        //};

        this.defaultChartOptions.chart.spacingTop = this.drillDowns
            ? this.drillDowns.length > 0
                ? 200
                : this.viewType === 'advanced'
                    ? 50
                    : this.defaultChartOptions.chart.spacingTop
            : this.defaultChartOptions.chart.spacingTop;
        if (this.tileType === 'map' && this.showMap) {
        ...
        if (this.selectedPlaceTypeInput !== undefined && this.tileType === 'map') {
            let thisScope = this;
            window.setTimeout(() => {
                thisScope.onSelectedMapViewChange(thisScope.selectedPlaceTypeInput);
            }, 1500);
        }
    }
    ...
    ngOnChanges(changes: any) {
        ...
        if (changes.selectedPlaceTypeInput && this.tileType === 'map' && this.mapChart) {
            this.onSelectedMapViewChange(changes.selectedPlaceTypeInput.currentValue);
        }
    }
